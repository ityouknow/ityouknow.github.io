...
我们在熔断示例项目spring-cloud-consumer-hystrix的基础上更改，
...
http://localhost:9001/hystrix.stream ，输入之后点击 monitor，进入页面，如果没有访问会先显示```Loading ...```。直接访问http://localhost:9001/hystrix.stream 也会不断的显示ping 这样的字符。

访问服务http://localhost:9001/hello/neo，就可以看到监控的效果了，首先访问http://localhost:9001/hystrix.stream，显示如下：
